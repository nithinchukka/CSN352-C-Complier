...
void addDeclaratorsToSymbolTable(ASTNode *a, ASTNode* b) {
    // Collect all specifiers from a's children
    ...
    for (size_t i = 0; i < b->children.size(); i++) {
        ASTNode* child = b->children[i];
        if (child && !child->children.empty() && child->children[0]) {
            ASTNode* firstChild = child->children[0];

            if (ASTNode::nodeTypeToString(firstChild->type) == "ARRAY") {
                string arrayName = firstChild->children[0]->valueToString();
                string arraySize = firstChild->children[1] ? firstChild->children[1]->valueToString() : "";
                string arrayType = typeSpecifiers + "[" + arraySize + "]";
                addToSymbolTable(arrayName, arrayType);

            } else {
                addToSymbolTable(firstChild->valueToString(), typeSpecifiers);
            }
        }
    }
}
...
direct_declarator
	...
	| direct_declarator LPAREN RPAREN { $$ = $1; }
	;
